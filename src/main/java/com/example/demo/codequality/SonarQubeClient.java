        ...
        if (responseCode == HttpURLConnection.HTTP_OK) {
            ...
            if (jsonResponse.has("issues") && jsonResponse.get("issues").isArray()) {
                ...
                for (JsonNode issue : issues) {
                    ...
                    // Example: Use OpenRewrite to apply a static analysis recipe

 Recipe recipe = new org.openrewrite.staticanalysis.CommonStaticAnalysis();
                    org.openrewrite.ExecutionContext ctx = new org.openrewrite.InMemoryExecutionContext();
                    org.openrewrite.SourceFile sourceFile = ...; // Load the source file related to the issue
                    List<Result> results = recipe.run(List.of(sourceFile), ctx);
                    results.forEach(result -> {
                        System.out.println("Fixed issue in file: " + result.getAfter().getSourcePath());
                    });

Recipe recipe = new org.openrewrite.java.cleanup.CommonStaticAnalysis();
                    org.openrewrite.ExecutionContext ctx = new org.openrewrite.InMemoryExecutionContext();
                    ...
                    List<Result> results = recipe.run(sourceSet, ctx);
                }
